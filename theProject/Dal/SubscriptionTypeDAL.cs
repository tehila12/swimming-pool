...
namespace Dal
{
    public class SubscriptionTypeDAL
    {
        ...
        //בדיקה האם סוג המנוי קיים כבר במערכת
        public static bool CheckIfExist(Subscription_type st)
        {
            try
            {
                using (Swimming_PoolEntities db = new Swimming_PoolEntities())
                {
                    var c = db.Subscription_type.FirstOrDefault(p => p.type == st.type);
                    if (c != null)
                        return true;
                    else
                        return false;
        ...
        //מספר כניסות לפי איידי סוג מנוי
        public static int SumEnterDal(int type)
        {
            try
            {
                ...
                using (Swimming_PoolEntities db = new Swimming_PoolEntities())
                {
                    ...
                    return (int)t.Num_of_entrances;
                }

            }
            catch (Exception e)
            {
                return -1;    //לבדוק אם -1 או משהו אחר
            }
        }
