...
function GetTheSubObj() {
    ...
    axios.get(path + 'Subscribers/GetSubByCustId/' + custId).then(
        (response2) => {
            ...
            if (subObj.gender == 'זכר') {
                if (sessionStorage.CurrenGender == 'נשים')
                    alert("הבריכה פתוחה כרגע לנשים")
            }
            else if (subObj.gender == 'נקבה') {
                if (sessionStorage.CurrenGender == 'גברים')
                    alert("הבריכה פתוחה כרגע לגברים")
            }
                ...
                //הוספת כניסת מנוי
                axios.post(path + 'CustEnter/PostSub/' + subId + '/' + CurrentShift).then(
                    (response3) => {
...
function confirmRemove() {
    ...
    if (id != null) {
        ...
        if (confirm("האם אתה בטוח שברצונך למחוק?")) {
            axios.delete(path + 'CustEnter/Delete/' + id).then(
                (response) => {
                    console.log(response)
                    var result = response.data;
            ...
            //אם הכניסה נמחקה, צריך לבדוק אם הוא מנוי ולעדכן את מספר הכניסות שלו
